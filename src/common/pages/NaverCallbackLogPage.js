...
class NaverCallbackPage extends Component {
  ...
  componentDidMount() {
    ...
    let naverLogin = new window.naver.LoginWithNaverId({
      ...
      // callbackHandle 설정 시 callbackUrl로 요청된 현재 페이지 callback 처리
      callbackHandle: true,
      // callback 페이지가 분리되었을 경우에 callback 페이지에서는 callback 처리를 해줄 수 있도록 설정
    })
    // 설정 정보를 초기화하고 연동을 준비
    naverLogin.init();
    ...
    window.addEventListener('load', function () {
      naverLogin.getLoginStatus(function (status) {
        ...
        if (status) {
          ...
          // 요청 성공 시 네이버 로그인 정보를 담은 로그인이벤트를 동작하기 위한 url 설정
          // 로그인에 필요한 이메일을 파라미터 함수로 사용
          let url = "/login?"+email;

          // 별도의 팝업으로 호출된 callback 페이지 닫기.
          window.self.close();
          // App.js의 BY NAVER 로그인 부분인 <Route exact path="/login/:email" component={LoginPage} /> 호출
