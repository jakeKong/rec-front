...
class UserRegister extends Component {
  ...
  componentDidMount() {
    ...
    const { user, popupOpened } = this.props;
    if (popupOpened  === undefined) {
      ...
      // 권한 선택 왼쪽필드 (선택안함 상태)
      if (lsbBeforeAssignedRoles.firstElementChild === null) {
      ...
      if (user === undefined) {
        ...
        if (user.assignedRoles !== null && user.assignedRoles.length !== 0) {
          dto.assignedRoles = user.assignedRoles._tail.array;
          user.assignedRoles.forEach(e => {
            roleCodeItems.forEach(role => {
              if (e === role.value) {
                const roleCodeItem = document.createElement('vaadin-item')
                roleCodeItem.textContent = role.textContent;
